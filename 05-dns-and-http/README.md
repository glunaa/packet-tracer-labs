...
| R3     | Fa0/1     | 10.2.2.2/30     |
| R3     | S0/0/0    | 10.4.4.2/30     |
| PC1    | NIC       | 10.1.1.10/24    |
| PC2    | NIC       | 10.3.3.10/24    |

*Simplified 3-router chain: R1 → R2 → R3 for primary path, R1 → R3 serial for backup.*
...
```ios
! Route to PC2's network (10.3.3.0/24) via R2
ip route 10.3.3.0 255.255.255.0 10.2.2.2

! Default route — send all unknown traffic to R2
ip route 0.0.0.0 0.0.0.0 10.2.2.2

! Floating static route (backup via serial, higher AD = 5)
! Only used if primary route goes down
...
> **Administrative Distance (AD):** Lower = more preferred. Connected = 0, Static = 1. Setting AD to 5 on the backup ensures the primary (AD 1) is always preferred while it's up. This is a **floating static route**.
...
! Default route toward internet (example)
...
! Test path
traceroute 10.3.3.10                 ! From R1 — shows hops
```

**From PC1:**
```
ping 10.3.3.10      ✅  (PC1 → PC2 via R1 → R2 → R3)
tracert 10.3.3.10       (shows hop path)
```

**Failover test:**
1. Shut down R1's Fa0/1: `interface fa0/1` → `shutdown`
2. Run `show ip route` on R1 — primary route disappears
3. Floating static route (via serial) takes over
